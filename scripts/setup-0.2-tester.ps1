...
# Versions this plugin (v1.2.2) was tested against — do not float.
$TLC_CORE = "3lc==3.1.0"
...
# will materialize the plugin's venv (id kaggle-exdark, version 1.2.2).
# Derivation rule (SDK worker_spec.py): TLC_COMPUTE_PLUGIN_VENV_ +
# id.upper().replace('-', '_').
$managed = Join-Path $env:USERPROFILE ".3lc-compute\managed-plugins\kaggle-exdark\1.2.2\.venv\Scripts\python.exe"
