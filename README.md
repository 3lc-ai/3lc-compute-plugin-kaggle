...
**Version pairing (v1.2.11):** 3LC Hub with `3lc-compute==0.2.1` + `3lc==3.1.0`
(the 0.2.x plugin platform), or `3lc-compute==1.0.1` + `3lc==3.3.0` (the 1.0.x
line). v1.2.11 installs on both — see [1.0.x deltas](#10x-deltas) for the three
...
[1.0.x deltas](#10x-deltas) right after it. Same plugin, same `@v1.2.11` tag,
