# Remote compute — browser and services on different machines
...
is the per-surface verdict on every browser==host assumption in the plugin,
against `3lc-compute 0.2.1` + `3lc 3.1.0`.
...
- Dashboard deep links must carry `?object_service=<url>` or the dashboard
  silently falls back to its own `localhost:5015`. The plugin appends it on
  every outbound link (`kgWithObjectService`); a new link that skips it is a
...
| Dashboard links (Explore, Open Run, Loop inspect/fix-labels, Status run cell) | all append `object_service=` |
...
### Host paths the UI must keep labelling

These work remotely, but read as browser-local unless the UI says otherwise.
The labelling is the fix; removing it re-creates the confusion.

| Surface | Why it needs a label | What the UI says |
|---|---|---|
...
No hardcoded `localhost` exists in the plugin's shipped code, and Download CSV
streams bytes rather than referencing a path — both are invariants worth
re-checking whenever a new outbound link or artifact surface is added.

### Broken / not supported remotely — big, out of plugin scope

| Gap | Detail |
|---|---|
| No file upload for the starter kit | nothing in the browser can put the kit on the host; the Import tab's Download starter kit section is the supported route, and it runs in the host's worker |
...
- `UV_TORCH_BACKEND=auto` is required for CUDA torch in shop installs
  (uv detects the box's NVIDIA driver and picks the matching CUDA wheel index)
