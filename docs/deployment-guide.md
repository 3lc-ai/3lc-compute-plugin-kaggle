...
sidebar HTML/JS — grep for `display_mode` across `site-packages\**\*.js` returns nothing).
So the contract, as observed: the frontend takes `display_mode == "sidebar"` manifests and
groups them under uppercased `section` headers. The three entries you see under **AI
TOOLS** are exactly the three built-ins that declare `section = "AI Tools"`:
...
`routes/plugins.py:44-171`) — long-running jobs require our own routes, like yolo's
...
`tlc_compute/routes/plugins.py:183-220` (JWT required; `?force=true` to reload past
...
re-fetches `/api/plugins/`), and the UI fragment (fresh `get_ui_fragment()`, cache cleared
— keep the read-`ui.html`-into-`_ui_cache` pattern, `sam3/__init__.py:54-67`).
