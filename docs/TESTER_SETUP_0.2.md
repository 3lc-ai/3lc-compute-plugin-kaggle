# Tester setup — 0.2.x Hub + Kaggle plugin v1.2.4 (catalog install)
...
>    `managed-plugins\kaggle-exdark\1.2.4\`.
>
...
# version 1.2.4 — the env-var name is TLC_COMPUTE_PLUGIN_VENV_ +
...
| Kaggle page 500s on first open | W1 env var not set in the compute-service window (see step 2), or set to a wrong path — it must point at `...\managed-plugins\kaggle-exdark\1.2.4\.venv\Scripts\python.exe`. Round-1 machines: the OLD name `TLC_COMPUTE_PLUGIN_VENV_KAGGLE` no longer does anything. |
