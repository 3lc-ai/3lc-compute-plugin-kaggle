...
> `kaggle_run_20260722_150451`, Kaggle ref 54911899), because the hidden
> test split is harder than val. The organizer yolo11x reference submission
...
epochs. Ten epochs is roughly 2 to 5% of a from-scratch schedule.
...
losses, because the model can't fit noise that contradicts the images. This
model is learning normally; it's just at the very start of the curve.
(Precision stays ~0.003 because an uncertain model emits many low-confidence
boxes; recall is the informative early signal.)
...
which independently confirms that no class index is misrouted; an off-by-one
in the label map would crater specific classes, not scale all of them.
...
- **The competition workspace's 20-epoch from-scratch baselines** (YOLOv8n,
  `train_baselines.py`, workspace root, not in this repo) on five other
  datasets landed anywhere from mAP50 0.04 to 0.93 at epoch 20 depending on
  dataset difficulty. Early from-scratch numbers are dominated by dataset
  hardness, and ExDark (low-light, 5.9k train images, 12 classes) is on the
  hard end.
- Published from-scratch runs typically converge around epochs 200 to 300;
...
- The observed curve is pre-liftoff: mAP50 roughly doubled over epochs 5→10
  ...
  once precision starts moving; expect it in the 15-to-30-epoch range on this
  dataset, with rapid gains after.

Expectation for the competition's productive range (**50 to 150 epochs**, as
the Tips page says): a plain from-scratch yolo11n run should land somewhere in
the **~0.2 to 0.45 mAP50** band, with data-curation work (the point of the
...
So: **10 epochs ≈ 0.004 is on-curve.** 50 to 150 epochs lands a respectable
...
- A static line under **Epochs**: from-scratch runs score near-zero mAP50
  for the first 10 to 20 epochs by design.
...
3. Run `control_pretrained_sanity.py` (competition workspace root) for the
   pretrained control. (Under the 2026-07-22 contract this is simply what a
   plugin run does, but still never submit its outputs directly: submissions
   must come from plugin-trained runs with recorded provenance.)
