...
- **ETA recompute rule** — the trainer remeasures ETA at every epoch end (whole-run average: elapsed ÷ epochs done × epochs left), never per poll; the UI shows it only while running, from epoch 1 to the last. The pre-run duration hint is separate: median `avg_epoch_s` of the 5 most recent finished train jobs on this machine.
