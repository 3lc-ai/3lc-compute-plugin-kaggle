...
- **shared project name (project follow)** — the Import tab's Project field is the single source of truth (`kgCurrentProject`); on tab entry, Train/Predict re-point empty or other-project table URLs at that project's defaults (`kgSyncFieldToProject`) so a renamed project never strands later tabs on the old one (round-1 lesson); hand-entered URLs within the current project are never touched. Since v1.2.1.
...
- **config persistence rule** — the Import form persists as it settles (debounced input + blur), not only when an import runs; a settled project change re-points and re-persists the train/submit URL fields (`kgSyncFieldToProject` saves what it rewrites) and re-runs their gates, so `ui_config.json` always holds one coherent project (round-2 finding: typed-project loss + v122check/v124check2 cross-project mixture; exit_gate_125 C1 asserts coherence). Run-start still snapshots its own tab. Since v1.2.5.
...
- **tags** — `vX.Y.Z`; the version string is identical in pyproject.toml, plugin.toml, and the catalog entry; never retag. Current release: **v1.2.4** (tagged 2026-08-12; device saved-'0' migration, derived __version__ footer fix, polish batch a-d).
